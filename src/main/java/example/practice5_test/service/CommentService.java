...
public class CommentService {
    ...
    public boolean addComments(CommentDto commentDto){
        ...
        // 엔티티에 게시물 ID(FK) 넣기  >> 얘를 저장 먼저 하면 안됨??
        savedCommentEntity.setBoardEntity(boardRepository.findById(commentDto.getBoardId()).orElse(null));

        // 결과 확인(댓글 엔티티 id여부로)
        if (savedCommentEntity.getCommentId() >= 1) {
            return true;
        } else {
            return false;
        }
